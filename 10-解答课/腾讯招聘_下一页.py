...
class TencentSpider(object):
    def __init__(self):
        self.next_url = 'https://hr.tencent.com/position.php?&start=#a0'
    ...
    def start_work(self):
        page_number = self.next_url.split('=')[1].split('#a')[0]
        if page_number == '':
            print('正在爬取第%s页' % 1)
        else:
            print('正在爬取第%s页' % int((int(page_number)/10+1)))


        item = {}
        content = requests.get(url=self.next_url,headers = self.headers).content.decode()
        html = etree.HTML(content)
        self.next_url = 'https://hr.tencent.com/' + ''.join(html.xpath('//a[@id="next"]/@href'))
        if self.next_url.endswith('javascript:;') == True:
            return
        node_list = html.xpath('//*[@class="event"]|//*[@class="odd"]')
        if node_list == []:
            return
